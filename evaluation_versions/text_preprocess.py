...
class txt_preprocesser():
    ...
    def removeRapWords(self,wordList):
        rapsw = {}
        with open('rapsw.txt') as f:
            f = f.read()
            f = f.split()
            rapsw = set(f)
        
        wordList[:] = [word for word in wordList if word not in rapsw]
        # LIST COMP to remove any words that are also found in the dictionary of stopwords
        # new list returned has NO STOPWORDS
        return wordList
    ...
    def stemWords(self,wordList):
         ...
         while i < len(wordList):
             wordList[i] =  self.get_lemma(wordList[i])
             i += 1
         return wordList


    def process(self,text,addition):
        ...
        if addition == 'rp':
            listwdocid = self.removeRapWords(listwdocid)
